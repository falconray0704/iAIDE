    ...
    while ((i=db_scan())!=TNEWLINE)
    {
        switch (i)
        {
            case TID :
            {
                ...
                *db_order=(DB_FIELD*)
                realloc((void*)*db_order, ((*db_osize)+1)*sizeof(DB_FIELD));
                ...
                for (l=0;l<db_unknown;l++)
                {
                    ...
                    {
                        if (check_db_order(*db_order, *db_osize, db_value[l])==RETFAIL)
                        {
                ...
                for (l=0;l<db_alias_size;l++)
                {
                    ...
                    {
                        if (check_db_order(*db_order, *db_osize, db_aliasvalue[l])==RETFAIL)
                        {
                            error(0,"Field %s redefined in @@dbspec\n",dbtext);
                            (*db_order)[*db_osize]=db_unknown;
                        }
...
int db_writespec_file(db_config* dbconf)
{
    ...
    if((key=get_db_key())!=NULL)
    {
        ...
        dbconf->do_dbnewmd=1;
        if( (dbconf->dbnewmd=
                    mhash_hmac_init(dbconf->dbhmactype, key, keylen, mhash_get_hash_pblock(dbconf->dbhmactype)))== MHASH_FAILED)
        {
...
int db_writeacl(acl_type* acl,FILE* file,int a)
{
    ...
    else
    {
        ...
        {
            dofprintf(",%i,%i,%i", acl->acl[i].a_type, acl->acl[i].a_id, acl->acl[i].a_perm);
        }
    }
#endif
