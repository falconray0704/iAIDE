...
{
    return (((old!=NULL && new!=NULL) && strcmp(old,new)!=0 ) ||
...
static int has_md_changed(byte* old,byte* new,int len)
{
    ...
    return (((old!=NULL && new!=NULL) && (bytecmp(old,new,len)!=0)) ||
            ((old!=NULL && new==NULL) || (old==NULL && new!=NULL)));
}
...
{
    if (a1->a_type!=a2->a_type || a1->a_id!=a2->a_id || a1->a_perm!=a2->a_perm)
    {
        return RETFAIL;
    }
    return RETOK;
...
static seltree* get_seltree_inode(seltree* tree, db_line* file, int db)
{
    ...
    /* found the match */
    if((db == DB_NEW && tree->new_data != NULL && file->inode == tree->new_data->inode) ||
            (db == DB_OLD && tree->old_data != NULL && file->inode == tree->old_data->inode))
    {
        return tree;
    }
...
seltree* new_seltree_node( seltree* tree, char*path, int isrx, rx_rule* r)
{
    seltree* node=NULL;
    seltree* parent=NULL;
    char* tmprxtok = NULL;
...
void gen_seltree(list* rxlist,seltree* tree,char type)
{
    ...
    for(r=rxlist;r;r=r->next)
    {
        ...
        if(curnode==NULL)
        {
            seltree * newNode = NULL;
            curnode=new_seltree_node(tree,rxtok,1,curr_rule);
            newNode=get_seltree_node(tree,rxtok);
 ...
 */
static void add_file_to_tree(seltree* tree,db_line* file,int db, DB_ATTR_TYPE attr)
{
    seltree* node=NULL;
